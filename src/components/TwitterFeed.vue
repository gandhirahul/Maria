<template>
  <div class="twitter-feed">
    <Loader v-if="loading" />
    <TweetList ref="tweetList" v-else :tweets="tweets" />
...
export default class TwitterFeed extends Vue {
  ...
  getPastTweets() {
    const timeStamp = this.tweets[this.tweets.length - 1].timeStamp;

    tweetService
      .getTweetsBeforeAfterTime(timeStamp)
  ...
  handleScroll() {
    const tweetListHeight = this.$refs.tweetList.clientHeight;
    ...
    if (window.scrollY >= tweetListHeight) {
      this.getPastTweets();
    }
  }
  ...
  updateFeed() {
    return tweetService
      .getTweetsCreatedAfterTime(this.latestTweetTimestamp)
